...
class SindyAutoencoder(tf.keras.Model):
    ...
    def compile(self,
                ...
                **kwargs):
        super(SindyAutoencoder, self).compile(optimizer=optimizer, loss=loss, **kwargs)
    ...
    def train_step(self, data):
        ...
        # Split gradients
        n_autoencoder_vars = len(encoder_vars) + len(decoder_vars)
    ...
    #     # gradients = tape.gradient(loss, trainable_vars)
    #     # self.optimizer.apply_gradients(zip(gradients, trainable_vars))
    #     ## Apply different gradients for autoencoder and sindy
