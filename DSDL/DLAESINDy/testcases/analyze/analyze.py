...
def load_results(name, path='./results/'):
    ...
    custom_objects = {
        "SindyAutoencoder": lambda config: SindyAutoencoder(params, **config),
        "Sindy": Sindy
    }
    try:
        print(f"{path}{name}")
        model = tf.keras.models.load_model(f"{path}{name}/model.keras")
        print("loaded")
...
if __name__ == '__main__':
    ...
    path = "./path/to/your/results/"
    model, params, results = load_results(name, path)
    name_list = get_checkpoint_names("./path/to/checkpoints/")


    cases = get_cases(path, filter_case=['lorenz'])
    name_list = get_names(cases, path)
    non_existing_models, non_existing_params, remove_files = read_results(name_list, path, start_time=6, end_time=30, 
                            threshold=1e-2, t0_frac=0.0, end_time_plot=30, display_params=None, query_remove=False, known_attractor=False)
    # delete_results(remove_files, path)
    print('non existing models: ', non_existing_models)
    print('non existing params: ', non_existing_params)
    print('removed files: ', remove_files)